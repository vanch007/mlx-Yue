...
from .common import fresh_directory, read_json, write_json
...
def options(args):
    result = dict(model=args.model, vae=args.vae, local_files_only=args.offline,
                  precision=args.precision, memory_budget_gib=args.memory_budget_gib)
    if args.revision:
        result["revision"] = args.revision
    if args.vae_revision:
        result["vae_revision"] = args.vae_revision
    return result
...
def decode(args, output, loader):
    ...
    argv = ["replay", str(args.source), "--stage", "decode", "--output", str(output / "song"),
            "--model", args.model, "--vae", args.vae, "--precision", args.precision,
            "--memory-budget-gib", str(args.memory_budget_gib)]
    if args.offline:
        argv.append("--offline")
    main(argv)
    receipt = read_json(output / "song/result.json")
    return {"status": "complete", "output": str(output / "song"),
            "identity": receipt["identity"], "truncated": receipt["truncated"]}
    ...
    try:
        ...
        with YuE2Pipeline.from_pretrained(**loader) as pipe:
            for request in requests:
                destination = fresh_directory(output / (request.cot if args.action == "all-modes" else args.action))
                write_json(output / (request.cot + "-input.json"), request.to_dict())
                try:
                    if args.action == "plan":
