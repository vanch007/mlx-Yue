...
## Local MLX adaptation (2026-09-13)

This checkout runs through `lyra` / `yue2-mlx` and `from lyra import YuE2Pipeline`.
Use `yue2-mlx music`, `yue2-mlx abc`, and `yue2-mlx listen` for the packaged helpers.
Use `yue2-mlx transcribe` and `yue2-mlx cover` for the native audio-to-score workflow.
Model paths are in `models/paths.json`. Select `--precision bf16` for the reference-quality baseline.
The generation runtime and default VAE use native MLX. Torch is only a development/reference dependency.
Music helper outputs retain invocation metadata at the root and native artifacts under `generate/`, `plan/`, or the selected mode.
Local acceptance status is in `docs/IMPLEMENTATION.md`; do not inherit upstream or historical quality claims.
...
Read the local [README](../../README.md) for installation and models/paths.json for this machine's paths.
Install this checkout with `uv sync --no-dev --extra transcription`.
Generation and native transcription share this Torch-free environment. The upstream
Transformers examples in [models-and-setup.md](references/models-and-setup.md) are reference-only.
Download the pinned public model snapshots and record their revisions.
This skill's original instructions, helpers, and templates are
...
Use the local baseline: one request at a time, Apple Silicon, BF16 generation,
native FP32 VAE, default YuE2 settings. Do not silently shorten a requested song or lower inference
