...
with YuE2Pipeline.from_pretrained(
    model, vae=listening_vae, precision="bf16", local_files_only=True,
) as pipe:
...
For a Hub load, use `from_pretrained(repo, revision=model_revision, vae=vae_repo, vae_revision=vae_revision, precision="bf16")` with independently verified revisions. `cache_dir`, `token` and `local_files_only` are supported. Keep tokens in the authentication environment, out of scripts and manifests.
...
with YuE2Pipeline.from_pretrained(
    model, vae=evaluation_vae, precision="bf16", local_files_only=True,
) as decoder:
