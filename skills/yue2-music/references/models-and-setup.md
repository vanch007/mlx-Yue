...
> Local adaptation: use `yue2-mlx prepare` for YuE2, and `yue2-mlx transcribe`
> for native MLX SheetSage2/MERT2. Install this checkout with
> `uv sync --no-dev --extra transcription`. These share one Torch-free runtime.
> The Transformers/CUDA examples retained below are upstream reference material,
> not installation or execution instructions for the local MLX branch.
