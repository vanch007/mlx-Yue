...
try:
    with GPUExecution(backend="mps", memory_budget_gib=16, require_ac=True,
        ...
        with CapturedPipeline(model, vae, device="mps", backend="torch-eager",
            ...
            if source_report.get("weights") != pipe.weights or metadata.get("weights") != pipe.weights:
                raise ValueError("Saved semantic stage lacks matching original model/VAE identities")
            report["source_weights_match"] = True
